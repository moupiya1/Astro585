...
int main()
{
clock_t tic = clock();         //This starts the clock counter. I wanted it set up to measure performances.
#pragma omp parallel          // This parallelizes the process via openmp.
{
...
clock_t toc = clock();
    ...
    printf("Elapsed: %e seconds\n", (double)(toc - tic) / CLOCKS_PER_SEC); //Prints time taken for calculation. This one takes .01 sec n average, whereas the serial version takes almost no time . This must be because of the small data set size and overhead costs.
}
